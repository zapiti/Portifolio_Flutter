...
public class SwiftSpeechRecognitionPlugin: NSObject, FlutterPlugin, SFSpeechRecognizerDelegate {
  ...
  private func stopRecognition(result: FlutterResult) {
    ...
    @objc fileprivate func didFinishTalk() {
    ...
    self.timer = Timer.scheduledTimer(timeInterval: 2, target: self, selector: #selector(self.didFinishTalk), userInfo: nil, repeats: true)
    ...
    self.timer = Timer.scheduledTimer(timeInterval: 2, target: self, selector: #selector(self.didFinishTalk), userInfo: nil, repeats: true)
      ...
      if let result = result {
        ...
        self.myResult = result.bestTranscription.formattedString;
